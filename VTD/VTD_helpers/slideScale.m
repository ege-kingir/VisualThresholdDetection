function [position, RT, answer] = slideScale(screenPointer, question, rect, anchors, params, varargin)

%% currently used for the VTD task
% 30.11.2023
% % EK addition: I hard coded the screenNum into 2 for now.
...
responseKeys  = [KbName('1!') KbName('3#') KbName('2@')];
% GetMouseIndices;
drawImage     = 0;
...
t0                         = GetSecs;
answer                     = 0;
iter = 0; %checks for how many times the 'enter' button is pressed.
while answer == 0
    ...
    DrawFormattedText(screenPointer, question, 'center', rect(4)*0.35); 
    ...
    if length(anchors) == 2
        ...
        DrawFormattedText(screenPointer, anchors{1}, leftTick(1, 1) - textBounds(1, 3)/2,  rect(4)*0.6, [],[],[],[],[],[],[]); % Left point
        DrawFormattedText(screenPointer, anchors{2}, rightTick(1, 1) - textBounds(2, 3)/2,  rect(4)*0.6, [],[],[],[],[],[],[]); % Right point
    else 
        % Left, middle and right anchors
        DrawFormattedText(screenPointer, anchors{1}, leftTick(1, 1) - textBounds(1, 3)/2,  rect(4)*0.6, [],[],[],[],[],[],[]); % Left point
        DrawFormattedText(screenPointer, anchors{2}, 'center',  rect(4)*0.6, [],[],[],[],[],[],[]); % Middle point
        DrawFormattedText(screenPointer, anchors{3}, rightTick(1, 1) - textBounds(2, 3)/2,  rect(4)*0.6, [],[],[],[],[],[],[]); % Right point
    end
...
%         [~, secs, keyCode] = KbCheck;
        [pressed, firstPress, firstRelease] = KbQueueCheck;
            ...
            [~,pressedKeys] = find(firstRelease);
            pressedKeys
            % Check if any of the response keys are pressed
            if any(ismember(pressedKeys, KbName(responseKeys(1))))
